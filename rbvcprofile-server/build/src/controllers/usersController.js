...
const updateUser = async (req, res) => {
    const { id, firstName, lastName, phone, email, roles, active, password } = req.body;
    // Confirm data
    if (!id ||
        !firstName ||
        !lastName ||
    ...
    if (password) {
        // Hash password
        user.password = await bcrypt_1.default.hash(password, 10); // salt rounds
    }
...
const deleteUser = async (req, res) => {
    ...
    const message = await Message_1.Message.findOne({ user: id }).lean().exec();
    if (message) {
